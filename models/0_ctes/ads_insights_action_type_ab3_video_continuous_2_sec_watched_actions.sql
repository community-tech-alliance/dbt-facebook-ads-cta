...
SELECT
    ...
    ,cast(json_extract_scalar(video_continuous_2_sec_watched_actions, "$['value']") as decimal) as value
    ,_airbyte_ads_insights_hashid
    ,_airbyte_emitted_at
FROM {{ ref('ads_insights_action_type_ab2') }}
CROSS JOIN unnest(video_continuous_2_sec_watched_actions) as video_continuous_2_sec_watched_actions
WHERE 1 = 1
