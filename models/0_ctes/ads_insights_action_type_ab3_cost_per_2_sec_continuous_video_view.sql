...
SELECT
    ...
    ,date_start
    ,date_stop
    ,cast(json_extract_scalar(cost_per_2_sec_continuous_video_view, "$['value']") as decimal) as value
    ,_airbyte_ads_insights_hashid
    ,_airbyte_emitted_at
FROM {{ ref('ads_insights_action_type_ab2') }}
CROSS JOIN unnest(cost_per_2_sec_continuous_video_view) as cost_per_2_sec_continuous_video_view
WHERE 1 = 1
